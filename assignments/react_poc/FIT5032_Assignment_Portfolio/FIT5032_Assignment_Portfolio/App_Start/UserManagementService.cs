...
using static FIT5032_Assignment_Portfolio.App_Start.RoleHelpers;
    ...
    public class UserManagementService : IUserManagementService
    {
        ...
        public async Task<int> GetAllUsersCountAsync(string searchString)
        {
            ...
            if (!string.IsNullOrEmpty(searchString))
                users = users.Where(user => (user.PreferredNameTitle.Contains(searchString)));
        ...
        public async Task<List<ApplicationUser>> GetUsersAsync(int offset, int limit, string sortOrder, string searchString)
        {
            ...
            if (!string.IsNullOrEmpty(searchString))
                pageUsers = pageUsers.Where(user => (user.PreferredNameTitle.Contains(searchString)));

            switch (sortOrder)
            {
                case "PreferredNameTitle":
                    pageUsers = pageUsers.OrderBy(u => u.PreferredNameTitle);
                    break;
                case "PreferredNameTitle_desc":
                    pageUsers = pageUsers.OrderByDescending(u => u.PreferredNameTitle);
                    break;
                case "Email":
